	...
	class citizen{
		...
		public function __set($name,$value){
			...
			if ($this->id != NULL) {
				switch($name){
					case "password":
						$db->query("update citizen set ".$name."=".($value===null?"NULL":"ENCRYPT('".$db->real_escape_string($value)."')").", reset_password_token=NULL where (id='".$this->id."')");
					break;
					default :
						$db->query("update citizen set ".$name."=".($value===null?"NULL":"'".$db->real_escape_string($value)."'")." where (id='".$this->id."')");
					break;
				}
			}
		}
		...
		public function __get($name){
			...
			if ($this->id != NULL) {
				switch($name){
					...
					break;
					case "isvalid":
						$q=$db->query("select count(*) from citizen where (id='".$this->id."')");
						$r=$q->fetch_row();
						return $r[0]==1;
					break;
					case 'displayname':
		...
		public static function create($username,$password){
			global $db;
			if(citizen::username_exists($username)) return array("status"=>"username_exists");
		...
		public function delete(){
			global $db;
			$db->query("delete from citizen where (id='".$this->id."')");
		}
		...
		public static function login($username, $password, $ip){
			...
			$q=$db->query("select id, password from citizen where (username='".$username."')");
			if($q->num_rows==0){
